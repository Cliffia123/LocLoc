...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        if x1.shape[-1] == 3:
            x1 = F.interpolate(x1, size=7, mode='bilinear', align_corners=False, )
            # x1 = torch.nn.Upsample()(x1)
        elif x1.shape[-1] == 1:
...
class UNet(nn.Module):
    def __init__(self, c_in, n_classes, c_base=4, activation='relu'):
        ...
        self.up_2 = Up(c_base * 6, c_base * 3, activation=activation) #(24, 12)
        self.up_3 = Up(c_base * 3 + c_in, n_classes, activation=activation) #(12+384, 200)

    def forward(self, x1): # 256, 384, 16, 16
        x2 = self.down_1(x1)  #256, 8, 8, 8
        x3 = self.down_2(x2)  #256, 16, 4, 4
        x4 = self.down_3(x3)  #256,2,2 or 3, 3
